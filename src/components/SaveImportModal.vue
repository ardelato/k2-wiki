        ...
        <div class="w-full max-w-lg rounded-xl border border-border bg-card p-5 shadow-2xl">
          ...
          <div v-if="state === 'upload'">
            ...
            <label
              ...
              :class="isDragging
                ...
                : 'border-border hover:border-accent/50 hover:bg-muted/30'" @dragenter.prevent="isDragging = true"
          ...
          <div v-else-if="state === 'preview'">
            <div class="mb-3 flex flex-wrap gap-3 text-sm">
              ...
              <span v-if="stats.newCount" class="rounded-md bg-emerald-500/15 px-2 py-1 font-medium text-emerald-400">
                +{{ stats.newCount }} new
              </span>
              <span v-if="stats.changed" class="rounded-md bg-amber-500/15 px-2 py-1 font-medium text-amber-400">
            ...
            <div class="max-h-[50vh] space-y-4 overflow-y-auto pr-1">
              <div v-for="[tier, group] in groupedByTier" :key="tier">
                ...
                <div class="space-y-1">
                  <div v-for="c in group" :key="c.id" class="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-sm"
                    :class="{
                      'bg-emerald-500/10': c.isNew,
                      'bg-amber-500/10': (c.levelChanged || c.awakenedChanged) && !c.isNew,
                    }">
                    <img v-if="getCreatureImage(c)" :src="getCreatureImage(c)" :alt="c.name"
                      class="size-7 rounded object-cover" />
                    <div v-else class="size-7 rounded bg-muted" />
                    <span class="flex-1 font-medium" :class="c.awakened ? 'text-pink-400' : ''">{{ c.name }}</span>
                    <span v-if="c.awakened" class="text-xs" :class="c.awakenedChanged ? 'font-semibold text-pink-300' : 'text-pink-400/70'">★ Awakened</span>
                    ...
                    <Check v-else-if="!c.levelChanged && !c.awakenedChanged" class="size-3.5 text-muted-foreground/50" />
                  </div>
                </div>
              </div>
            </div>
