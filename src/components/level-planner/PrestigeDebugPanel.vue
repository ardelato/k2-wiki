...
<template>
  ...
  >
      ...
      <template v-else>
        <div class="grid grid-cols-2 gap-2 text-xs">
          ...
          <div class="rounded-lg border border-border/40 bg-muted/30 px-3 py-2">
            ...
            <p class="font-mono font-semibold">{{ (plan.idleWasteFraction * 100).toFixed(1) }}%</p>
          </div>
        </div>
          ...
          <table class="w-full text-2xs">
            <tbody class="divide-y divide-border/20">
              <tr v-for="e in anchorsPerExpedition" :key="e.id">
                ...
                <td class="py-1 text-right font-mono text-muted-foreground/60">/{{ e.size }}</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <div v-else-if="activeTab === 'input'" class="flex flex-1 flex-col overflow-hidden">
      ...
      <pre class="flex-1 overflow-auto p-4 font-mono text-2xs leading-relaxed text-foreground">{{
        inputJson || 'No input.'
      }}</pre>
    </div>
