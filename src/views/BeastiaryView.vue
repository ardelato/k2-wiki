...
const { isOwned, setOwned, getLevel, toggleOwned, setLevel, ownedCreatureIds } =
...
function setAllOwned(value: boolean) {
  for (const c of displayCreatures.value) {
    setOwned(c.id, value);
  }
}
...
const jobEntries = computed(
  () => Object.entries(jobLabels) as [keyof Jobs, string][],
);
const statEntries = computed(
  () => Object.entries(statLabels) as [keyof CreatureStats, string][],
);
...
const sortedCreatures = computed(() => {
  ...
  list.sort((a, b) => {
    ...
    if (key === "name") result = a.name.localeCompare(b.name);
      ...
      result = (a.types[0] ?? "").localeCompare(b.types[0] ?? "");
...
function sortBy(key: SortKey) {
  if (tableSortKey.value === key) {
    tableSortDirection.value =
      tableSortDirection.value === "asc" ? "desc" : "asc";
    return;
  }
...
<template>
  <section class="space-y-5 lg:space-y-6">
    ...
    <div class="flex flex-wrap gap-2">
      <button
        class="focus-ring rounded-md border border-primary/40 bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary hover:bg-primary/20"
        @click="setAllOwned(true)"
      >
        Set all owned
      </button>

      <button
        class="focus-ring rounded-md border border-border px-3 py-1.5 text-xs font-semibold hover:bg-muted/50"
        @click="setAllOwned(false)"
      >
        Set all unowned
      </button>
    </div>
            ...
            <div
              ...
              :class="[
                ...
                selectedCreature?.id === creature.id
                  ? 'ring-2 ring-primary/60 border-primary/40 opacity-100'
                  : '',
                editing
                  ? ''
                  : 'hover:-translate-y-0.5 hover:opacity-100 hover:shadow-glow',
              ]"
              @click="
                editing ? toggleOwned(creature.id) : selectCreature(creature)
              "
            >
              <!-- Type gradient bar -->
              <div
              ...
              <!-- Owned check overlay -->
              <button
                class="absolute left-2 top-3 z-10 flex size-7 items-center justify-center rounded-full border shadow-md transition"
                :class="
                  isOwned(creature.id)
                    ? 'bg-primary text-primary-foreground border-primary'
                    : 'bg-card/80 text-muted-foreground border-border hover:bg-muted'
                "
                @click.stop="setOwned(creature.id, !isOwned(creature.id))"
              >
                <Check v-if="isOwned(creature.id)" class="size-3.5" />
              ...
              <!-- Type chips -->
              <div
                class="absolute right-1.5 top-5 z-10 flex flex-col items-end gap-0.5"
              >
                <span
              ...
              <div class="space-y-2 rounded-b-xl bg-card/80 px-3 pb-3 pt-2.5">
                <div class="text-center">
                  <p class="truncate text-lg font-extrabold text-foreground">
                    {{ creature.name }}
                  </p>
                  <div v-if="isOwned(creature.id)" class="space-y-1 px-1">
                    <div
                      class="flex items-center justify-between text-[10px] font-mono text-muted-foreground"
                    >
                      <span>LVL</span>
                      <span>{{ getLevel(creature.id) }}</span>
                    </div>
                ...
                <div
                  v-if="isOwned(creature.id) && editing"
                  class="space-y-1"
                  @click.stop
                >
                  <p
                  ...
                  >
                    ...
                    <input
                      ...
                      @blur="
                        normalizeCollectionLevelOnBlur(creature.id, $event)
                      "
                    />
                    <button
                      class="focus-ring inline-flex h-7 w-7 items-center justify-center text-muted-foreground transition hover:bg-muted/60 hover:text-foreground disabled:cursor-not-allowed disabled:opacity-40"
              ...
              <tr>
                ...
                >
                  ...
                  >
                    ...
                    <span
                      :class="
                        tableSortKey === 'name' ? 'text-primary' : 'opacity-0'
                      "
                ...
                >
                  ...
                  >
                    ...
                    <span
                      :class="
                        tableSortKey === 'tier' ? 'text-primary' : 'opacity-0'
                      "
                      >{{ tableSortDirection === "asc" ? "▲" : "▼" }}</span
                    >
                  </button>
                ...
                >
                  ...
                  >
                    ...
                    <span
                      :class="
                        tableSortKey === 'type' ? 'text-primary' : 'opacity-0'
                      "
                      >{{ tableSortDirection === "asc" ? "▲" : "▼" }}</span
                    >
                  </button>
                ...
                >
                  ...
                  >
                    ...
                    <span
                      :class="
                        tableSortKey === 'trait' ? 'text-primary' : 'opacity-0'
                      "
                      >{{ tableSortDirection === "asc" ? "▲" : "▼" }}</span
                    >
                  </button>
                ...
                >
                  ...
                  >
                    ...
                    <span
                      :class="
                        tableSortKey === jobKey ? 'text-primary' : 'opacity-0'
                      "
                      >{{ tableSortDirection === "asc" ? "▲" : "▼" }}</span
                    >
                  </button>
              ...
              >
                ...
                >
                  <div class="flex items-center gap-3">
                    <div
                      ...
                      :style="{
                        backgroundColor:
                          'hsl(' + typeColorVar(creature.types[0]) + ' / 0.1)',
                      }"
                    >
                      <img
                ...
                >
                  <div class="flex items-center gap-2">
                    <div
                      class="relative h-2 w-10 overflow-hidden rounded-full bg-muted/60"
                    >
                      <div
                        class="absolute inset-y-0 left-0 rounded-full"
                        :style="{
                          width:
                            (creature.jobs[jobKey] / maxJobLevel) * 100 + '%',
                ...
                <td
                  class="px-2 py-2.5 font-mono text-xs font-semibold text-foreground"
                >
                  {{ totalJobs(creature) }}
                </td>
              </tr>
        ...
        >
          ...
          >
            ...
            <img
              ...
              :style="{
                backgroundColor: `hsl(${typeColorVar(selectedCreature.types[0])} / 0.1)`,
              }"
            />
            ...
            <p class="mt-1 text-sm text-muted-foreground">
              T{{ selectedCreature.tier + 1 }} ·
              {{ toTitleCase(selectedCreature.mainJob) }}
            </p>
          ...
          <div class="space-y-5 px-5 pb-5">
            ...
            <section class="border-t border-border/60 pt-4">
              <h3
                class="mb-3 text-xs font-bold uppercase tracking-[0.16em] text-muted-foreground"
              >
                Collection
              </h3>
              <div class="space-y-3">
                ...
                >
                  ...
                  <button
                    ...
                    :class="
                      isOwned(selectedCreature.id) ? 'bg-primary' : 'bg-muted'
                    "
                ...
                >
                  ...
                  >
                    ...
                    <input
                      ...
                      aria-label="Creature level"
                      @blur="
                        normalizeCollectionLevelOnBlur(
                          selectedCreature.id,
                          $event,
                        )
                      "
            ...
            <section class="border-t border-border/60 pt-4">
              <router-link
                :to="{
                  path: '/planner',
                  query: { tab: 'levelup', creature: selectedCreature.id },
                }"
            ...
            <section class="border-t border-border/60 pt-4">
              <div class="mb-3 flex items-baseline justify-between">
                <h3
                  class="text-xs font-bold uppercase tracking-[0.16em] text-muted-foreground"
                >
                  Stats
                </h3>
                <span
            ...
            >
              ...
              <div class="space-y-2">
                <router-link
                  ...
                  :to="{
                    path: '/expeditions',
                    query: { expedition: entry.expedition.id },
                  }"
                ...
                >
                  <div class="flex items-center justify-between gap-2">
                    ...
                    >
                      ...
                      <img
                        v-if="
                          ...
                          getItemImage({
                            id: entry.expedition.rewards[0].itemId,
                          })
                        "
                        :src="
                          getItemImage({
                    ...
                    <span
                      class="shrink-0 font-mono text-xs font-semibold text-primary"
                      >{{ entry.score }}%</span
                    >
                  </div>
                  <div
                  ...
                  >
                    ...
                    >
                    <span
                      v-if="entry.biomeStatus === 'advantage'"
                      class="text-green-500"
                      >· ▲ Advantage</span
                    >
                    <span
                      v-if="entry.biomeStatus === 'disadvantage'"
            ...
            <section class="border-t border-border/60 pt-4">
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <span
                    v-else
                    class="size-1.5 shrink-0 rounded-full bg-accent/60"
                  />
